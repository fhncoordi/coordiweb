...
WHERE titulo = 'Parlamento Canario de la Mujer con Discapacidad'
...
WHERE titulo = 'TAIDA - Radio Escolar e Inclusión Educativa'
...
WHERE titulo = 'TENIQUE 2024 - VI Edición'
  AND area_id = (SELECT id FROM areas WHERE slug = 'participaca');
...
WHERE titulo = 'Proyecto DRACAENA'
...
UPDATE proyectos
SET descripcion = 'Programa integral de formación para la inserción laboral que incluye certificados de profesionalidad, especialidades propias en diversos sectores (turismo, tecnología, sanitario, agrario, energías renovables) y formación a la carta adaptada a empresas colaboradoras.

Este programa combina formación profesional de calidad con itinerarios personalizados, favoreciendo la empleabilidad y el acceso al mercado laboral de personas con discapacidad.'
WHERE titulo = 'DRACAENA 14 - Acciones formativas'
...
WHERE titulo = 'Análisis de impacto de la teleasistencia avanzada'
  AND area_id = (SELECT id FROM areas WHERE slug = 'forminno');
